...
def init():                                   
    f = open("/home/lgy/twitter_api/twitter_USA/code/more_user_list.csv")
    for line in f:
        line = line.strip()
...
def main():
    ...
    f = open('/./user_follower/user_follower_0.json','w')
    while True:
        count +=1
        user_id = user_id_list[id_index]
        oauth = get_oauth(key_index)
        
        url = "https://api.twitter.com/1.1/followers/ids.json?user_id="+user_id
        ...
        if(r.status_code == 429):            
            ...
            if count % 1000 == 0:
                f.close()
                f = open('./user_follower/user_follower_'+str(count/1000)+'.json','w')
            f.write(str(user_id)+'\n')
            f.write(str(data))
            f.write('\n')
            id_index +=1
